...
export const SSQ = {
  ...
  calcY: function (jd: number) {
    const A = [], // 中气 ZQ
      B = [] //日月合朔表(整日) HS
    const dx = new Array(14)
    const ym = new Array(14)
    var i, W, w // k,
    ...
    // const ZQ_PE1 = this.calc(W - 15.2, '气')
    // const ZQ_PE2 = this.calc(W - 30.4, '气') //补算二气,确保一年中所有月份的“气”全部被计算在内
    ...
    var YY = floor((A[0] + 10 + 180) / 365.2422) + 2000 //确定年份
    if (YY >= -721 && YY <= -104) {
      var ns = new Array(),
        yy
      for (i = 0; i < 3; i++) {
      ...
      for (i = 0; i < 14; i++) {
        ...
        f1 = floor((B[i] - ns[nn] + 15) / 29.5306) //该月积数
